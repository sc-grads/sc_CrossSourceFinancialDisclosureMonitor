...
def search_grouped(user_input: str, min_similarity: float = 0.75):
    """
    Semantic grouped search:
    1) Find best semantic match from embeddings
    2) Negation mismatch guard (unless similarity is very high)
    ...
    with get_conn() as conn:
        with conn.cursor() as cur:
            ...
            # allow contradiction only if super-high confidence
...
def search_with_auto_ingest(user_input: str, min_similarity: float = 0.75) -> dict:
    ...
    # Resolve a symbol from the user's headline
